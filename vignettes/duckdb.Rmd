...
For functions that cannot be translated, duckplyr falls back to the original R implementation,
disrupting the DuckDB pipeline and materializing intermediate results.

Furthermore, DuckDB has functions with no R equivalent.
These might be used already by code that interacts with DuckDB through dbplyr,
either making use of its passthrough feature (unknown functions are translated to SQL verbatim),
or by using the `mutate(x = sql(...))` pattern.
...
It achieves this by creating a temporary view that points to the relational object created internally by duckplyr,
in the same DBI connection as the duckplyr object.
...
The escape hatch, also introduced in duckplyr 1.1.0,
allows calling arbitrary DuckDB functions directly from duckplyr, without going through SQL:
...
See `vignette("limits")` for limitations in the translation employed by duckplyr,
`vignette("fallback")` for more information on fallback,
and `vignette("telemetry")` for existing attempts to prioritize work on the translation layer.
